...
SOURCE = ROOT / "course-package" / "materials" / "chinese-cooking-c-exam.md"
...
EXTRA_REASONS = {
    "黃豆": "黃豆在食品分類上屬豆類，營養分類通常歸入豆魚蛋肉類，不是核桃、腰果、杏仁這類堅果種子。",
    "乳酸菌": "乳酸菌會利用糖類進行乳酸發酵並產生乳酸，乳酸就是泡菜酸味的主要來源。",
    "修飾澱粉": "修飾澱粉具有增稠與穩定質地的作用，能讓醬油膏比一般醬油更濃稠。",
    "紅燒": "紅燒需要醬色與醬香，深色醬油能提供上色與較濃的醬香，最符合此烹調法。",
    "有害物質": "油脂長時間高溫加熱會氧化、裂解並產生劣變物質，因此不是延長保存或改善品質。",
    "乳酸": "乳酸是發酵酸味的主要化學來源，能直接解釋題幹中的酸味現象。",
    "碳酸鈉": "碳酸鈉屬鹼性物質，是食用鹼或鹼水常見的主要成分之一。",
    "氯化鉀": "氯化鉀不是製作豆腐常用的凝固劑；豆腐常用凝固劑包含硫酸鈣、氯化鎂或葡萄糖酸內酯等。",
...
def analysis_for(topic, question, options, correct_letters, correct_text, is_multiple):
    ...
    if is_multiple:
        return (
            ...
            f"中餐烹調丙級學科的分類、操作條件、衛生規範或法規要求。正確選項之所以成立，是因為它們都"
            f"符合題幹指定的條件；其餘選項（{wrong_text}）則至少有一項概念、對象、時間、溫度、用途或"
            f"法規條件不符。複習時應把正確選項拆成獨立規則背誦，再用錯誤選項反向確認常見混淆點。"
        )
...
"""
    ...
    for item in items:
        ...
        source = (
            "勞動部勞動力發展署技能檢定中心，中餐烹調丙級技術士技能檢定學科測試參考資料；"
...
tag: 中餐葷食丙級
title: 中餐葷食丙級學科測試參考資料
desc: 115.07 版中餐葷食丙級學科題庫，共 {total} 題，涵蓋食物性質、選購、貯存、製備、排盤裝飾、器具設備、營養、成本控制、衛生知識與衛生法規。
time: 120 分鐘
questions: 共 {total} 題
score: 滿分 100 分
