...
SOURCE = ROOT / "course-package" / "materials" / "chinese-cooking-111-official-exam.md"
OUTPUT = ROOT / "content.md"

ANSWER_INDEX = {"A": 0, "B": 1, "C": 2, "D": 3}
UNIT_ID = "day1.u-1"
TOPIC = "111 學年度 H 中餐烹飪學科試題"

SOURCES = (
    "111 學年度商業類學生技藝競賽 H 中餐烹飪學科試題；"
...
def parse():
    ...
    while i < len(lines):
        ...
        if number == 41:
            answer_letters = ["B"]
        question = clean_text(match.group(3))
        options = []
        i += 1
        ...
        while i < len(lines):
            ...
            if opt_match:
                option_text = re.sub(r"[。．\s]+$", "", clean_text(opt_match.group(2)))
...
- 完成 111 學年度 H 中餐烹飪學科試題 {total} 題完整練習。
...
tag: 中餐烹調
title: 111 學年度 H 中餐烹飪學科試題
desc: 111 學年度商業類學生技藝競賽 H 中餐烹飪學科試題，共 {total} 題，涵蓋競賽規範、食品衛生、食材處理、米食點心、菜系文化與烹調技法。
time: 60 分鐘
questions: 共 {total} 題
score: 滿分 100 分
