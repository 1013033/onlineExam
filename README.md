...
- 線上測驗：支援作答、即時批改、詳解顯示、計時、進度保存、手機接續。
- 錯題庫試作：目前已先在 `100-baking/` 試作，答錯會自動進入錯題庫，錯題重練連續答對 2 次後自動出庫。
...
目前 `100-baking/index.html` 額外包含錯題庫試作功能。
...
後續若確認功能符合需求，可把同一套邏輯套用到其他分年度試題與彙總試題。
